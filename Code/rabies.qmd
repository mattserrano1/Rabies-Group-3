...
```{r}
rabies |> group_by(
  Received.Year) |> summarise( 
  HighSamples = sum(Number.of.Samples))
```
...
```{r}
rabies |> filter(
  Animal.Classification == "Bat") |> filter(
    Positive.Samples != 0)
...
```{r}
rabies |> select(
  Animal.Classification, Number.of.Samples, Percent_pos) |> group_by(
    Animal.Classification) |> filter(
    Number.of.Samples>4, Percent_pos>50)
```
...
SLC_animals <- SLC |> group_by(
  Animal.Classification) |> summarise(
    Total_Sub = sum(Number.of.Samples),
    Pos_Sub = sum(Positive.Samples),
    Percent_pos = (Pos_Sub/Total_Sub)*100) |>
  View()
```
